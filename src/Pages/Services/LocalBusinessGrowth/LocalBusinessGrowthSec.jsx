...
const LocalBusinessGrowthSec = () => {
  ...
  return (
    <Bounce duration={2500}>
      <section className={`local-features-section transition-colors duration-300 ${
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {localFeatures.map((item, idx) => (
              <motion.div 
                ...
                className={`local-feature-card group transition-colors duration-300 ${
                  theme === 'light' ? 'bg-white/90 border border-slate-200 shadow-sm' : ''
                }`}
              >
                <div className={`local-feature-icon-box transition-colors duration-300 ${
                  theme === 'light' ? 'text-amber-600' : ''
                }`}>
                  {item.icon}
                </div>
                <h3 className={`local-feature-title transition-colors duration-300 ${
                  theme === 'light' ? 'text-slate-900' : ''
                }`}>
                  {item.title}
                </h3>
                <p className={`local-feature-desc transition-colors duration-300 ${
