...
const HeroSecFive = () => {
  ...
  return (
    <section className="relative overflow-hidden industries-section">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* Section Heading */}
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xs sm:text-sm font-semibold uppercase tracking-wider mb-4">
        ...
        {/* 10 Grid Cards Layout */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6">
          {industriesList.map((item, index) => (
            <Link
              key={index}
              to={item.path}
              className="bg-slate-900/60 backdrop-blur-xl border border-white/10 hover:border-amber-500/50 rounded-2xl p-5 text-center flex flex-col items-center justify-between min-h-[190px] shadow-lg transition-all duration-300 hover:-translate-y-1.5 hover:bg-slate-900/90 hover:shadow-amber-500/15 group no-underline"
            >
              {/* Icon Badge */}
