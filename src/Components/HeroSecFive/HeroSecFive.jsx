...
const HeroSecFive = () => {
  ...
  return (
    <section className="relative overflow-hidden industries-section py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <motion.div 
            ...
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xs sm:text-sm font-semibold uppercase tracking-wider mb-4 shadow-lg shadow-amber-500/10"
          >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6">
          {industriesList.map((item, index) => (
            <motion.div
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.4, delay: index * 0.08 }} // Har card thore delay se line mein aayega
              whileHover={{ y: -8, scale: 1.02, transition: { duration: 0.2 } }}
              className="h-full"
            >
              <Link
                to={item.path}
                className="bg-slate-900/60 backdrop-blur-xl border border-white/10 hover:border-amber-500/50 rounded-2xl p-5 text-center flex flex-col items-center justify-between min-h-[190px] shadow-lg transition-colors duration-300 hover:bg-slate-900/90 hover:shadow-xl hover:shadow-amber-500/15 group no-underline h-full"
              >
                {/* Icon Badge */}
                ...
                {/* Title & Stats */}
                <div>
                  <h3 className="text-xs sm:text-sm font-bold text-slate-100 mb-1.5 tracking-wider uppercase group-hover:text-amber-300 transition-colors">
                    {item.title}
                  </h3>
                  <p className="text-xs text-slate-400 group-hover:text-slate-300 flex items-center justify-center gap-1">
