...
const HeroSecSeven = () => {
  ...
  return (
    ...
    <section className="relative overflow-hidden testimonials-ticker-section py-12">
       <Zoom duration={1500}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center mb-14 relative z-10">
        
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-500/15 border border-amber-500/30 text-amber-400 text-xs sm:text-sm font-semibold uppercase tracking-wider mb-4 hero-badge">
        ...
        <p className="testimonials-subtitle mx-auto mb-6">
      ...
      <div className="marquee-wrapper py-4">
        <div className="marquee-track-left space-x-6 px-3">
          {[...testimonials, ...testimonials].map((item, index) => (
            <div 
              key={index}
              className="testimonial-card w-80 sm:w-96 p-6 flex flex-col justify-between"
            >
              <div>
                ...
                <p className="testimonial-text text-sm mb-6 italic">
                  "{item.review}"
                </p>
              </div>

              <div className="flex items-center gap-3 pt-4 border-t border-white/5">
                <div className="author-avatar shrink-0 flex items-center justify-center font-bold">
                  {item.name ? item.name.charAt(0) : 'U'}
                </div>
                <div className="text-left">
                  <h4 className="text-sm font-bold text-slate-100 mb-0.5">{item.name}</h4>
                  <p className="x-small text-slate-400">{item.role}</p>
                </div>
              </div>
            </div>
          ))}
